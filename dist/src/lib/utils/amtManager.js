...
export class AMTManager {
    ...
    async getPowerState() {
        try {
            const response = await this.makeRequest('http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_PowerManagementService/GetPowerState', this.createGetPowerStateRequest());
            const text = await response.text();
